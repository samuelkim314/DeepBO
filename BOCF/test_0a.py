...
kernel = GPy.kern.SE(input_dim=2, variance=2., lengthscale=0.3)
cov = kernel.K(grid)
mean = np.zeros((6 ** 4,))
for j in range(m):
...
n_attributes = m
model = multi_outputGP(output_dim=n_attributes, exact_feval=[True] * m, fixed_hyps=True)
...
parameter_support = np.empty((1,3))
for j in range(3):
    def marginal_func(x):
    ...
    for x0 in starting_points:
        ...
        if best_val_found > res[1]:
            ...
            marginal_opt = res[0]
    parameter_support[0,j] = -best_val_found
...
max_iter = 50
for i in range(1):
    filename = './experiments/test_CBO1' + str(i) + '.txt'
    mabo = ma_bo.ma_BO(model, space, objective, acquisition, evaluator, initial_design, expectation_utility=expectation_U)
