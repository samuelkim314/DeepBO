...
class uKG_cf(AcquisitionBase):
    ...
    def __init__(self, model, space, optimizer=None, cost_withGradients=None, utility=None, expectation_utility=None, normalize=False):
        self.optimizer = optimizer
        self.utility = utility
        self.expectation_utility = expectation_utility
        self.normalize = normalize
        super(uKG_cf, self).__init__(model, space, optimizer, cost_withGradients=cost_withGradients)
                ...
                for l in range(L):
                    for W in self.W_samples:
                        ...
                        def inner_func_w_gradient(X_inner):
                            ...
                            func_val[:, 0] = self.expectation_utility.func(utility_params_samples[l], a, b)
                            func_gradient[0, :] = np.matmul(self.expectation_utility.gradient(utility_params_samples[l], a, b), g)
            ...
            for i in range(len(X)):
                ...
                for l in range(len(utility_params_samples)):
                    for W in W_samples2:
                        ...
                        g = np.concatenate((c, d))
                        marginal_dacq_dX[i, :, l] += np.matmul(self.expectation_utility.gradient(utility_params_samples[l], a, b), g)
