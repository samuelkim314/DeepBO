...
parameter_support = np.atleast_2d([1.25, 1.50, 1.25, 1.50])
...
def U_func(parameter, y):
    aux = y.transpose() - parameter
    return 100*np.squeeze(np.prod(norm.cdf(aux.transpose()), axis=0))


def dU_func(parameter, y):
    aux = np.squeeze(y) - parameter
    normal_cdf_aux = norm.cdf(aux)
    m = len(aux)
    gradient = np.empty((m,))
    for j in range(m):
        gradient[j] = norm.pdf(aux[j])*np.prod(np.delete(normal_cdf_aux, j))
    return 100*gradient
