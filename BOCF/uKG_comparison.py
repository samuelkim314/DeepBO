...
class uKG_comparison(AcquisitionBase):
    """
    ...
    def __init__(self, model, space, optimizer=None, cost_withGradients=None, utility=None, expectation_utility=None,
                 normalize=False):
        self.optimizer = optimizer
        self.utility = utility
        self.expectation_utility = expectation_utility
        self.normalize = normalize
        super(uKG_comparison, self).__init__(model, space, optimizer, cost_withGradients=cost_withGradients)
        ...
        self.W_samples = np.random.normal(size=(5, self.n_attributes))
        self.Z_samples = np.random.normal(size=(5, self.n_attributes))
    ...
    def _compute_acq(self, X, parallel=True):
        ...
        acqX2 = self._compute_acq2(X)
        print(acqX)
        print(acqX2)
        X = X[0, :]
        acqX, dacq_dX = self._compute_acq_withGradients(X)
        acqX2, dacq_dX2 = self._compute_acq_withGradients2(X)
        print(acqX)
        print(acqX2)
        print(dacq_dX)
        print(dacq_dX2)
                ...
                for l in range(L):
                    for W in self.W_samples:
                        ...
                        def inner_func(X_inner):
                            ...
                            func_val = np.empty((X_inner.shape[0], 1))
                            ...
                            b = self.model.posterior_variance_conditioned_on_next_point(X_inner)
                            func_val[:, 0] = self.expectation_utility.func(utility_params_samples[l], a, b)
                            return -func_val
                        ...
                        def inner_func_w_gradient(X_inner):
                            X_inner = np.atleast_2d(X_inner)
                            func_val = np.empty((X_inner.shape[0], 1))
                            func_gradient = np.empty(X_inner.shape)
                            cross_cov = self.model.posterior_covariance_between_points_partially_precomputed(X_inner,
                                                                                                             x)[:, :, 0]
                            dcross_cov_dX_inner = self.model.posterior_covariance_gradient_partially_precomputed(
                                X_inner, x)[:, 0, :]
                            ...
                            d = self.model.posterior_variance_gradient_conditioned_on_next_point(
                                X_inner)[:, 0, :]
                            g = np.concatenate((c, d))
                            func_val[:, 0] = self.expectation_utility.func(utility_params_samples[l], a, b)
                            func_gradient[0, :] = np.matmul(
                                self.expectation_utility.gradient(utility_params_samples[l], a, b), g)
        ...
        for l in range(L):
            for W in self.W_samples:
                ...
                def inner_func_w_gradient(X_inner):
                    ...
                    g = np.concatenate((c, d))
                    func_val[:, 0] = self.expectation_utility.func(utility_params_samples[l], a, b)
                    func_gradient[0, :] = np.matmul(self.expectation_utility.gradient(utility_params_samples[l], a, b),
                                                    g)
                    return -func_val, -func_gradient
    ...
    def _compute_acq_withGradients(self, X):
        ...
        if False:  # self.use_full_support:
            acqX = np.matmul(marginal_acqX, self.utility_prob_dist)
            ...
            for i in range(len(X)):
                ...
                for l in range(len(utility_params_samples)):
                    for W in W_samples2:
                        ...
                        a += np.multiply(cross_cov.T, aux).T
                        b = self.model.posterior_variance_conditioned_on_next_point(x_opt)
                        ...
                        d = -2 * np.multiply(dcross_cov_dx.T, tmp).T + np.multiply(
                            dvar_dX[:, i, :].T, np.square(tmp)).T
                        g = np.concatenate((c, d))
    ...
    def _compute_acq2(self, X, parallel=False):
        """
        Computes the aquisition function
    ...
    def _marginal_acq2(self, X, utility_params_samples):
        """
        """
        n_w = self.W_samples.shape[0]
                ...
                for l in range(L):
                    for W in self.W_samples:
                        ...
                        def inner_func_w_gradient(X_inner):
                            ...
                            dcross_cov_dX_inner = self.model.posterior_covariance_gradient_partially_precomputed(
                                X_inner, x)[:, 0, :]
                            dposterior_var_conditioned_on_x_dX_inner = self.model.posterior_variance_gradient_conditioned_on_next_point(
                                X_inner)[:, 0, :]
                            dposterior_std_conditioned_on_x_dX_inner = 0.5 * np.multiply(
                                np.reciprocal(posterior_std_conditioned_on_x), dposterior_var_conditioned_on_x_dX_inner)
                            a = self.model.posterior_mean(X_inner)
                            if False:
                                print('test begin')
                                x_aux = X_inner
                            ...
                            for Z in self.Z_samples:
                                ...
                                func_gradient += np.matmul(self.utility.eval_gradient(utility_params_samples[l], a + b),
                                                           c + d)
                            return -func_val, -func_gradient

                        # marginal_acqX[i,l] -= self.optimizer.optimize(f =inner_func)[1]
                        marginal_acqX[i, l] -= \
                        self.optimizer.optimize_inner_func(f=inner_func, f_df=inner_func_w_gradient)[1]

        marginal_acqX /= (self.n_hyps_samples * n_w * n_z)
        return marginal_acqX

    def _marginal_acq_parallel2(self, X):
        """
        """
        n_x = X.shape[0]
        marginal_acqX = np.zeros((X.shape[0], len(self.utility_params_samples)))
    ...
    def _parallel_acq_helper2(self, args):
        """
        """
        utility_params_samples = self.utility_params_samples
        L = len(utility_params_samples)
        ...
        for l in range(L):
            for W in self.W_samples:
                ...
                def inner_func(X_inner):
                    X_inner = np.atleast_2d(X_inner)
                    func_val = np.zeros((X_inner.shape[0], 1))
                    cross_cov = self.model.posterior_covariance_between_points_partially_precomputed(X_inner, x)[:, :,
                                0]
                    posterior_std_conditioned_on_next_point = np.sqrt(
                        self.model.posterior_variance_conditioned_on_next_point(X_inner))
                    a = self.model.posterior_mean(X_inner)
                ...
                def inner_func_w_gradient(X_inner):
                    ...
                    posterior_std_conditioned_on_x = np.sqrt(
                        self.model.posterior_variance_conditioned_on_next_point(X_inner))
                    dcross_cov_dX_inner = self.model.posterior_covariance_gradient_partially_precomputed(X_inner, x)[:,
                                          0, :]
                    dposterior_var_conditioned_on_x_dX_inner = self.model.posterior_variance_gradient_conditioned_on_next_point(
                        X_inner)[:, 0, :]
                    dposterior_std_conditioned_on_x_dX_inner = 0.5 * np.multiply(
    ...
    def _compute_acq_withGradients2(self, X):
        """
        """
        X = np.atleast_2d(X)
        # Compute marginal aquisition function and its gradient for every value of the utility function's parameters samples
    ...
    def _marginal_acq_with_gradient2(self, X, utility_params_samples):
        """
        """
        X = np.atleast_2d(X)
        marginal_acqX = np.zeros((X.shape[0], len(utility_params_samples)))
            ...
            for i in range(len(X)):
                ...
                for l in range(len(utility_params_samples)):
                    for W in W_samples2:
                        ...
                        c = np.multiply(dcross_cov_dx.T, aux).T + np.multiply(dvar_dX[:, i, :].T, aux2).T
                        tmp = cross_cov[:, 0] * inv_varX_noiseless[:, i]
                        dposterior_std_conditioned_on_x_dx = (-np.multiply(dcross_cov_dx.T, tmp).T + 0.5 * np.multiply(
